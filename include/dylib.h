...
class dylib {
   private:
    ...
    static HINSTANCE open_lib(const char *path) noexcept { return LoadLibraryA(path); }
    FARPROC get_symbol(const char *name) const noexcept { return GetProcAddress(m_handle, name); }
    void close_lib() noexcept { FreeLibrary(m_handle); }
    static char *get_error_message() noexcept {
        constexpr size_t bufSize = 512;
        auto errorCode = GetLastError();
        if (!errorCode) return nullptr;
    ...
    static void *open_lib(const char *path) noexcept { return dlopen(path, RTLD_NOW | RTLD_LOCAL); }
    void *get_symbol(const char *name) const noexcept { return dlsym(m_handle, name); }
    void close_lib() noexcept { dlclose(m_handle); }
    static char *get_error_message() noexcept { return dlerror(); }
#endif
    static std::string get_handle_error(const std::string &name) {
        auto err = get_error_message();
        if (!err) return "error while loading dynamic library \"" + name + "\"";
        return err;
    }
    static std::string get_symbol_error(const std::string &name) {
        auto err = get_error_message();
        if (!err) return "error while loading symbol \"" + name + "\"";
        return err;
    }
   ...
   public:
     ...
     */
    explicit dylib(const char *path) { open(path); }

    explicit dylib(const std::string &path) { open(path.c_str()); }

    dylib(std::string path, const char *ext) { open(std::move(path), ext); }
    ...
    void open(const char *path) {
        close();
        if (!path) throw handle_error(get_handle_error("(nullptr)"));
        m_handle = open_lib(path);
        if (!m_handle) throw handle_error(get_handle_error(path));
    }
    ...
    void open(std::string path, const char *ext) {
        close();
        if (!ext) throw handle_error("bad extension : (nullptr)");
        path += ext;
    ...
    std::function<T> get_function(const char *name) const {
        if (!m_handle) throw handle_error("error : no dynamic library loaded");
        if (!name) throw symbol_error(get_symbol_error("(nullptr)"));
        auto sym = get_symbol(name);
        if (!sym) throw symbol_error(get_symbol_error(name));
